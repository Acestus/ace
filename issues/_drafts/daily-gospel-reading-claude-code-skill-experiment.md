...
USCCB.org and prints it in chat, backed by its own dedicated Azure Function,
**so that** I can learn what it actually takes to build and publish a skill
to a Claude Skills hub (e.g. claudeskills.info), separate from the ACE-33/34
mobile app work.
...
- [ ] The skill is packaged and published so it can be downloaded from a
      Claude Skills hub (e.g. claudeskills.info or equivalent submission
      path).
...
3. **Claude Code skill: definition and script**
   - Research the Claude Code skill format (SKILL.md-equivalent) and what
     claudeskills.info or the relevant hub expects for submission/packaging
...
5. **Publish to the hub**
   - Package/submit the skill per whatever process claudeskills.info (or
