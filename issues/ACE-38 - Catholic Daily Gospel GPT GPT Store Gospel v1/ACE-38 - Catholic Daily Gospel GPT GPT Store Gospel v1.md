...
- [ ] A new standalone GitHub repo `catholic-daily-gpt` exists, containing a
      ...
      backends).
- [ ] The Azure Function scrapes/parses [USCCB.org](<http://USCCB.org>) and returns today's Gospel
...
- [ ] Verified end-to-end: asking the live, publicly listed GPT for today's
      reading returns the real, current Gospel text sourced from [USCCB.org](<http://USCCB.org>).
...
6. **Verification**
   ...
   * Confirm it returns the real, current Gospel citation + text sourced
     from [USCCB.org](<http://USCCB.org>) via the deployed Function.
...
WORKLOG: Stub created from Linear ACE-38
